...
| `size-N` / `size-full` / `size-1/2` / `size-[20px]` / `size-[50%]` | Sets BOTH width and height in one token (Tailwind v3.4+). Works on a childless `WDiv` (e.g. a `size-2 rounded-full` status dot). A later `w-*` / `h-*` overrides the matching axis. |
