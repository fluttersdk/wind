...
- `shrink-0` now sets `FlexFit.tight` so the child keeps its intrinsic size and does not shrink. Previously the token was accepted by `canParse` but set no `flexFit` (a silent no-op); `shrink` was unaffected.
