...
- `ZIndexParser.canParse` now rejects invalid `z-*` classes (e.g. `z-invalid`, `z-foo`) instead of returning `true` for any `z-` prefix; fixes silent no-op when `parse` cannot match the suffix. (#81)
