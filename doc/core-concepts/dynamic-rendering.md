...
> [!NOTE]
> `WDatePicker` delivers its value differently. It writes the date to the store like the others, but its action dispatches with no injected `_value`, so a handler reads the selected date from `state.get(id)`.
